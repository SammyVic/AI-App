...
class TestFmtGb:
    ...
    def test_zero(self):
        """This modal test meticulously validates the session loading and configuration dialogs ensuring that user inputs are correctly captured and validated before being passed to the core scanning and reporting engines."""
...
class TestStatsDialogData:
    ...
    def test_all_stats_keys_present(self):
        ...
        stats = {'total_runs': 5, 'total_files_scanned': 1000, 'total_duplicate_groups': 50, 'total_duplicate_files': 200, 'total_files_deleted': 100, 'total_delete_operations': 12, 'total_space_freed_bytes': 2 * 1024 ** 3, 'last_run_date': '2026-01-01', 'last_run_deleted': 10, 'last_run_space': 512 * 1024 * 1024}

        def _fmt_mb(b):
            return f'{b / 1024 ** 2:.2f} MB'
        items = [('Total Scans Run', f"{stats.get('total_runs', 0):,}"), ('Total Files Scanned', f"{stats.get('total_files_scanned', 0):,}"), ('Duplicate Groups Found', f"{stats.get('total_duplicate_groups', 0):,}"), ('Duplicate Files Found', f"{stats.get('total_duplicate_files', 0):,}"), ('Files Deleted', f"{stats.get('total_files_deleted', 0):,}"), ('Delete Operations', f"{stats.get('total_delete_operations', 0):,}"), ('Total Space Freed', sd_module._fmt_gb(stats.get('total_space_freed_bytes', 0))), ('Last Run Date', str(stats.get('last_run_date', 'Never'))), ('Last Run Files Deleted', f"{stats.get('last_run_deleted', 0):,}"), ('Last Run Space Freed', _fmt_mb(stats.get('last_run_space', 0)))]
        assert len(items) == 10
    ...
    def test_empty_stats_uses_defaults(self):
        """This modal test meticulously validates the session loading and configuration dialogs ensuring that user inputs are correctly captured and validated before being passed to the core scanning and reporting engines."""
        stats = {}
        items_values = [f"{stats.get('total_runs', 0):,}", f"{stats.get('total_files_scanned', 0):,}", sd_module._fmt_gb(stats.get('total_space_freed_bytes', 0)), str(stats.get('last_run_date', 'Never'))]
