...
class TestDuplicateGroup:
    ...
    def test_create_group(self, repo):
        ...
        sess = repo.create_session('C:/test')
        grp = repo.create_group(session_id=sess.id, group_key='exact_0', match_type='exact_hash', file_paths=['C:/a.txt', 'C:/b.txt'], space_recoverable_bytes=1024)
    ...
    def test_update_agent_decision(self, repo):
        ...
        repo._session.commit()
        repo.update_group_agent_decision(grp.id, recommended_keep='p', confidence=0.95, reasoning=[{'text': 'high similarity'}])
...
class TestFileMetadata:
    ...
    def test_add_and_bulk_add(self, repo):
        ...
        fms = [FileMetadata(session_id=sess.id, full_path='C:/b.txt', filename='b.txt', extension='.txt', size_bytes=200, modified_at=1235.0), FileMetadata(session_id=sess.id, full_path='C:/c.txt', filename='c.txt', extension='.txt', size_bytes=300, modified_at=1236.0)]
...
class TestLifetimeStats:
    ...
    def test_stats_after_completion(self, repo):
        ...
        repo.complete_session(s.id, 50, 3, 6, 2000000)
        repo.log_action('C:/a.txt', 'deleted', freed_bytes=500000)
        stats = repo.get_lifetime_stats()
        assert stats['total_runs'] == 1
        assert stats['total_files_scanned'] == 50
        assert stats['total_space_freed_bytes'] == 500000
...
class TestEngineBasics:
    ...
    def test_deduplicator_simple_match(self):
        ...
        mock_files = [FileInfo('C:/test/a.txt', 100, '.txt', 0), FileInfo('C:/test/copy/a.txt', 100, '.txt', 0), FileInfo('C:/test/b.txt', 200, '.txt', 0)]
    ...
    def test_deduplicator_fuzzy_match(self):
        """This general test case meticulously validates the core functionality of the specified test node within the Intelligent Dedup framework ensuring all expected side effects are accurately and reliably maintained."""
        config = ScanConfig(start_dir='C:/test', allowed_extensions={'.txt'})
        mock_files = [FileInfo('C:/test/Report_2023.txt', 100, '.txt', 0), FileInfo('C:/test/Report 2023.txt', 100, '.txt', 0), FileInfo('C:/test/unrelated.txt', 100, '.txt', 0)]
        with patch('app.engine.deduplicator.FileScanner') as MockScanner:
            scanner_inst = MockScanner.return_value
            scanner_inst.scan.return_value = mock_files
            with patch('app.engine.deduplicator.FileHasher') as MockHasher:
                hasher_inst = MockHasher.return_value
                hasher_inst.hash_batch.return_value = {'C:/test/file_v1.txt': 'hash1', 'C:/test/file_v1_fixed.txt': 'hash2', 'C:/test/unrelated.txt': 'hash3'}
