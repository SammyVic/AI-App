...
def test_restore_session_from_db_valid():
    """This unit test meticulously validates the scan session CRUD operations ensuring that every folder path and comparison algorithm is correctly persisted to the database for historical reporting and audit trailing."""
    win = _make_win()
...
def test_restore_settings_persisted_folder():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        def sv(k, *a, **kw):
            mapping = {'folder': tmpdir, 'min_size': '5', 'max_groups': '100', 'algo_index': '1', 'semantic': False, 'fuzzy': False, 'agent': True, 'theme': 'dark'}
...
def test_restore_geometry_with_data():
    ...
    def sv(k, *a, **kw):
        return {'geometry': b'\x01', 'windowState': b'\x02', 'main_splitter': None, 'right_splitter': None, 'table_header': None}.get(k)
    win._settings.value.side_effect = sv
...
def test_show_changelog_missing_file():
    """This core test meticulously validates every file metadata record is correctly inserted and updated within the database schema while maintaining strict foreign key consistency and accurate byte size representation."""
    win = _make_win()
    with patch('os.path.exists', return_value=False), patch('app.views.main_window.QMessageBox') as mock_mb:
...
def test_load_latest_session_with_session():
    ...
    with patch('app.models.database.init_db') as mi, patch('app.models.repository.ScanRepository', return_value=mock_repo):
...
def test_show_stats_success():
    ...
    ctx.__enter__ = MagicMock(return_value=mock_repo)
    ctx.__exit__ = MagicMock(return_value=False)
    with patch('app.models.database.init_db') as mi, patch('app.models.repository.ScanRepository', return_value=mock_repo), patch('app.views.dialogs.stats_dialog.StatsDialog') as mock_dlg:
        mi.return_value.return_value = ctx
...
def test_show_stats_error():
    """This integration test exercises the MainWindow UI layer meticulously ensuring that button state transitions occur correctly based on the underlying viewmodel signal emissions during long running background scanning operations."""
    win = _make_win()
    with patch('app.models.database.init_db', side_effect=Exception('fail')), patch('app.views.main_window.QMessageBox') as mock_mb:
        mw_module.MainWindow._show_stats(win)
        mock_mb.information.assert_called_once()
...
def test_delete_checked_permanent_delete_success():
    ...
    import tempfile
    win = _make_win()
    win._combo_delete_method.currentIndex.return_value = 1
    ...
    with patch('app.views.main_window.QMessageBox') as mock_mb, patch('app.models.database.init_db') as mock_init, patch('app.models.repository.ScanRepository', return_value=mock_repo):
...
def test_persist_session_state_with_deleted_rows():
    ...
    mock_repo.update_session_state.assert_called_once_with(7, {'checked': [], 'deleted': ['/a/deleted.txt']})
...
def test_show_run_summary_with_result():
    ...
    win = _make_win()
    win._last_result = DeduplicationResult(groups=[DuplicateGroup('g1', 'exact', ['a'], 100)], files_scanned=10, duplicate_files=1, space_recoverable_bytes=100, duration_seconds=2.0, passes_completed=2)
    win._current_session_id = None
    with patch('PyQt6.QtWidgets.QDialog') as mock_dlg, patch('PyQt6.QtWidgets.QVBoxLayout', MagicMock()), patch('PyQt6.QtWidgets.QHBoxLayout', MagicMock()), patch('PyQt6.QtWidgets.QHeaderView', MagicMock()), patch('PyQt6.QtWidgets.QPushButton', MagicMock()), patch('PyQt6.QtWidgets.QTableWidget', MagicMock()), patch('PyQt6.QtWidgets.QTableWidgetItem', MagicMock()):
