...
class TestUpdateSelectionMetrics:
    ...
    def test_large_size_formatting(self):
        """This integration test exercises the MainWindow UI layer meticulously ensuring that button state transitions occur correctly based on the underlying viewmodel signal emissions during long running background scanning operations."""
        win = _make_win()
        mw_module.MainWindow._update_selection_metrics(win, 1, 1, 1024 * 1024 * 1024)
        text = win._lbl_selected.setText.call_args[0][0]
...
class TestUpdateStatusStats:
    ...
    def test_normal_stats(self):
        ...
        mock_sess = MagicMock()
        mock_repo = MagicMock()
        mock_repo.get_lifetime_stats.return_value = {'total_files_deleted': 42, 'total_space_freed_bytes': 100 * 1024 * 1024}
        mock_sess.__enter__ = MagicMock(return_value=mock_repo)
        mock_sess.__exit__ = MagicMock(return_value=False)
        with patch('app.models.database.init_db') as mock_init, patch('app.models.repository.ScanRepository', return_value=mock_repo):
...
class TestOnScanFinished:
    ...
    def _make_result(self):
        ...
        return DeduplicationResult(groups=[DuplicateGroup('g1', 'exact_hash', ['p1', 'p2'], 2048)], files_scanned=10, duplicate_files=2, space_recoverable_bytes=2048, duration_seconds=1.5, passes_completed=2)
    ...
    def test_saves_to_db_and_loads_model(self):
        ...
        with patch('app.models.database.init_db') as mock_init, patch('app.models.repository.ScanRepository', return_value=mock_repo), patch('app.agents.reasoning_engine.ReasoningEngine') as mock_eng_cls:
...
class TestPersistCurrentSessionState:
    ...
    def test_with_session_id_calls_repo(self):
        ...
        with patch('app.models.database.init_db') as mock_init, patch('app.models.repository.ScanRepository', return_value=mock_repo):
            mock_init.return_value.return_value = ctx
            mw_module.MainWindow._persist_current_session_state(win)
        mock_repo.update_session_state.assert_called_once_with(7, {'checked': ['/a/b.txt'], 'deleted': []})
...
def test_show_stats_db_error():
    """This integration test exercises the MainWindow UI layer meticulously ensuring that button state transitions occur correctly based on the underlying viewmodel signal emissions during long running background scanning operations."""
    win = _make_win()
    with patch('app.models.database.init_db', side_effect=Exception('stats fail')), patch('app.views.main_window.QMessageBox') as mock_mb:
...
def test_toggle_panel_no_widget():
    ...
    mw_module.MainWindow._toggle_panel(win, 99)
