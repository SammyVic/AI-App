...
def test_on_finished_reset():
    ...
    result = DeduplicationResult(
        groups=[], files_scanned=10, duplicate_files=0, 
        space_recoverable_bytes=0, duration_seconds=1.0, passes_completed=2
    )
...
class TestDuplicateTableModel:
    ...
    def test_load_result(self):
        ...
        res = DeduplicationResult(
            groups=[
                DuplicateGroup("g1", "exact", ["C:/a.txt", "C:/b.txt"], 1000)
            ],
            files_scanned=2, duplicate_files=2, space_recoverable_bytes=1000,
        ...
        assert model.rowCount() == 3 # 1 header + 2 files
        assert model.columnCount() == 7
        
        # Check filename for a file row
    ...
    def test_checkbox_logic(self):
        model = DuplicateTableModel()
        res = DeduplicationResult(
            groups=[DuplicateGroup("g1", "e", ["a", "b"], 10)],
            files_scanned=2, duplicate_files=2, space_recoverable_bytes=10,
            duration_seconds=1, passes_completed=2
        )
        with patch("os.stat") as mock_stat:
    ...
    def test_mark_deleted(self):
        model = DuplicateTableModel()
        res = DeduplicationResult(
            groups=[DuplicateGroup("g1", "e", ["a"], 10)],
            files_scanned=1, duplicate_files=1, space_recoverable_bytes=10,
            duration_seconds=1, passes_completed=2
        )
        with patch("os.stat") as mock_stat:
            mock_stat.return_value.st_size = 100
    ...
    def test_get_checked_items(self):
        model = DuplicateTableModel()
        res = DeduplicationResult(
            groups=[DuplicateGroup("g1", "e", ["p1", "p2"], 10)],
            files_scanned=2, duplicate_files=2, space_recoverable_bytes=10,
            duration_seconds=1, passes_completed=2
        )
        with patch("os.stat") as mock_stat:
    ...
    def test_flags(self):
        ...
        # Mark deleted -> should not be checkable
        model.mark_deleted(1)
